...
이건 '더 빠른' 개발이 아니라, '더 책임감 있는' 개발을 위한 도구입니다.

오늘 머지한 코드, 몇 줄이나 읽으셨나요?

에이전트는 사람이 읽는 속도보다 빠르게 코드를 만듭니다.
그래서 병목은 타이핑이 아니라 사람의 이해입니다.
그런데 그 이해를 아무도 확인하지 않습니다. 그래서 우리는 거기부터 건너뜁니다.

KkochiKkochi 가 그 자리를 지킵니다.
에이전트가 커밋하려 하면 붙잡아 세우고 방금 뭘 바꿨는지 묻습니다.
대답하지 못하면 커밋을 통과시키지 않습니다.

Claude Code 와 Codex 를 지원합니다.

> 이 문서는 [README.md](README.md) 의 한국어판입니다. 정본은 영어판입니다. 두 문서가 어긋나면 영어판을 따르세요.
