...
아래는 실제 게이트 출력입니다. 퀴즈는 예시이며, 실제 문항은 변경마다 새로 생성됩니다.
...
Claude Code에서는 객관식이 `AskUserQuestion`으로 표시되어 클릭해서 답합니다. Codex에는 같은 UI가 없어 평문으로 출력되며 직접 입력해 답합니다. 문항 내용, 채점, 오답 재시도는 두 에이전트에서 동일합니다.

아직은 한국어만 지원합니다. 훅 메시지와 문항 생성 스킬이 모두 한국어를 기준으로 만들어져 있기 때문입니다. 다국어는 계획에 있습니다.
...
git 훅은 `.git/hooks/`에 설치됩니다. 이 디렉터리는 Git이 추적하지 않기 때문에 저장소마다 따로 설치해야 하며 `git clone`으로도 함께 오지 않습니다.
...
예외는 `core.hooksPath`를 사용하는 저장소입니다. 이 경우 실제 훅 디렉터리가 저장소에서 추적되는 위치이므로, 에이전트가 임의로 수정하지 않고 사용자에게 먼저 확인을 요청합니다([D32](docs/DECISIONS.md)).
...
문항은 최대 5개입니다. 원칙적으로는 최소 1개이며, 질문할 만한 내용이 전혀 없는 경우(예: lockfile 재생성, 포매팅만)에는 예외적으로 0개와 함께 사유를 기록하고 통과시킵니다.
...
- `commit` 명령일 때만 핸드셰이크를 남기고, 이 저장소에 git 훅이 설치되어 있는지와 버전이 최신인지 확인합니다.
