...
5. **오답에만 붙는 표지를 만들지 않는다.** 되묻기, 머뭇거림, 유독 짧거나 긴 선택지는 코드를 읽지 않고 소거하게 해 준다. 모든 선택지를 같은 문법과 비슷한 길이로 쓴다
...
한 문장에 하나만 묻는다. 코드는 문장으로 설명하지 말고 그대로 인용한다.

표로 부족하면 `humanize-korean` 스킬(im-not-ai)을 부른다 — **문항 텍스트만** 넘기고(코드 인용·경로·`evidence` 는 한 글자만 바뀌어도 근거가 어긋난다), 설치돼 있지 않으면 그냥 진행한다. 매 문항마다 부르지는 않는다([D46](../../docs/DECISIONS.md)).
