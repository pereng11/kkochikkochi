...
`PostToolUse` 가 요구한 경우, 그 요구가 알려준 `agent_id` 를 그대로 넘긴다.
...
- **종료 코드 1** — 스테이징된 변경이 정말 없다. 사용자에게 알리고 종료한다.
...
**설계 의도의 근거는 diff에 없다.** 대화에서 "왜 이 방식을 골랐는지"가 논의됐을 때만 그 축을 낸다.
...
- 상한 5는 네 축을 전부 담고 한 문항이 남는다. 남는 한 문항은 **영향·리스크** 축에 쓴다
...
2. **레포의 실제 문자열을 쓴다.** 실존하는 다른 경로·함수명에서 고른다
3. **선택지는 코드를 읽어야 가릴 수 있는 것으로 채운다.** "변함 없음"·"위 전부"·"해당 없음"은 읽지 않고도 소거되는 필러다. **"모르겠다"는 예외다** — §3에서 모든 문항에 상시 포함하는 탈출구이며, 후보 정답이 아니므로 정답으로 채점되지 않는다. 이 문서 전체에서 표기는 "모르겠다"로 통일한다
4. **정답 위치를 무작위화한다.** B·C 편향을 경계하라
5. **모든 선택지를 같은 문법과 비슷한 길이로 쓴다.** 되묻기·머뭇거림·길이 차이는 오답에만 붙는 표지가 되어, 코드를 읽지 않고도 소거하게 해 준다

### 근거를 짚을 수 있는 문항만 낸다

> 모든 문항은 정답 근거를 `파일:줄` 또는 `대화 내 발언`으로 특정할 수 있어야 한다.

특정하지 못한 문항은 **버린다.** 하드 게이트에서 가장 치명적인 실패는 정답이 틀린 것이다. 사용자가 맞는 답을 하는데 계속 오답 처리되면 진짜로 갇힌다.

근거는 각 문항의 `evidence` 필드에 기록한다.

### 문장은 읽는 즉시 이해돼야 한다

두 번 곱씹어야 뜻이 잡히는 문항은 **이해가 아니라 독해력을 잰다.** 아래는 [im-not-ai](https://github.com/epoko77-ai/im-not-ai) 의 룰북에서 이 도구의 문항에 자주 나오는 것만 추린 것이다. 전문은 `references/korean-sentences.md` 에 있다.

| 쓰지 말 것 | 대신 |
|---|---|
| ~에 대해, ~에 있어서, ~와 관련하여 | 조사로 직결한다 (X에 대해 검사 → X를 검사) |
| ~를 통해 남발 | ~로, ~해서 |
| ~에 의해 피동, ~되어진다 | 행위자를 주어로 (훅에 의해 기록된 → 훅이 기록한) |
| ~할 수 있다 남발 | 단언한다 (막을 수 있다 → 막는다) |
| 가지고 있다 | 형용사·동사로 (권한을 가지고 있다 → 권한이 있다) |
| 명사 앞 세 어절 넘는 관형구 | 문장을 자른다 |
| ~에서의, ~으로의 같은 이중 조사 | 절로 풀어쓴다 |
| 연결어미(-고 -며 -지만 -어서) 뒤 쉼표 | 쉼표를 뺀다 |

한 문장에 하나만 묻는다. 코드는 그대로 인용하되 **바꾸기 전 코드와 바꾼 뒤 코드를 나란히 붙인다** — "무엇이 바뀌었나" 축은 대조할 재료가 화면에 있어야 성립한다. 바꾸기 전 코드는 `git show HEAD:<path>` 로, 바꾼 뒤 코드는 `git cat-file -p <sha>` 로 얻는다(§1).

표로 부족할 때만 `humanize-korean` 스킬(im-not-ai)을 부른다([D46](../../docs/DECISIONS.md)) — **문항 텍스트만** 넘기고(코드 인용·경로·`evidence` 는 한 글자만 바뀌어도 근거가 어긋난다), 설치돼 있으면 쓰고 없으면 그대로 진행한다.
...
이 루프까지 포함해서 시간 목표가 3분이다. 오답이 나와도 루프를 끝까지 돈다.
...
오답      → 실제 코드를 인용해 해설한다
           → 같은 축, 다른 각도로 재출제
           → attempts 증가
           ※ 재출제는 반드시 다른 각도로 한다. 같은 문항을 되풀이하면
             이해가 아니라 정답을 외워서 통과한다
...
`questions` 는 0개일 때도 **반드시 빈 배열 `[]` 로 존재**해야 한다. 필드 자체를 빼거나 `null` 로 두면 `record-pass.sh` 가 "questions 는 배열이어야 합니다"로 거부한다. 문항이 0개일 때만 `skipped_reason` 에 사유를 적되, 한 글자 이상의 문자열로 적는다.
...
- `record-pass.sh` 는 퀴즈를 전부 통과한 뒤에만 부른다. 이 선이 게이트의 존재 이유다
- 정답은 사용자가 답한 뒤에 밝힌다
- 근거를 `파일:줄` 또는 `대화 내 발언`으로 짚을 수 있는 문항만 낸다
- 재출제는 같은 축, 다른 각도로 한다
