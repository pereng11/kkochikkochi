...
- **상한 3문항.** 억지로 채우지 않는다 — 근거 있는 문항만 낸다
...
- 1분 목표는 **전부 정답일 때** 기준. 오답 루프 진입 시 초과는 의도된 비용

**축은 4개인데 상한이 3문항이므로 하나는 반드시 탈락한다.** §7.3의 우선순위에 따라 하위 축부터 떨어뜨린다.

소요 시간 근거: 객관식 약 20초, 짧은 답 약 20초, 서술 한 문장 약 25초. 3문항이 약 60초.

### 7.3 축 우선순위와 출제 순서

우선순위는 아래와 같다. 상한 3문항에 걸리면 **낮은 축부터 탈락한다.**
...
| 4 | 재현 가능성 | 짧은 답 | 상시 후보. 위 축이 탈락하면 승격 |
...
```
설계 결정이 있는 작업    1 변경 사실 → 2 영향·리스크 → 3 설계 의도(서술, 마지막)
단순 버그 수정          1 변경 사실 → 2 영향·리스크 → 4 재현 가능성
고립된 소규모 변경      1 변경 사실
```
...
| 결정 | 선택 | 근거 |
|---|---|---|
| 게이트 강도 | 하드 차단 | 소프트 게이트는 병목을 해소하지 못함 |
| 검증 축 | 4축 전부 | 사실·영향·의도·재현 |
| 오답 처리 | 통과할 때까지 재출제 | 학습 효과 최대 |
| 탈출구 | "모르겠다" → 강제 학습 루프 | 무한루프 없이 우회도 없음 |
| 트리거 | 커밋 직전 (PreToolUse) | 경계가 명확하고 의미 단위가 큼 |
| 문항 형식 | 객관식 + 핵심 1문항 서술 | 마찰과 깊이의 균형 |
| 시간 목표 | 1분 (전부 정답 시) | 지속 가능성 |
| 문항 예산 | 상한 3, 억지로 채우지 않음 | 무의미한 문항이 신뢰를 깎음 |
| amend | 게이트 대상 | 새 내용물이 새는 경로를 막음 |
| revert | 통과 | 비상 레버를 막으면 사고가 커짐 |
| 토큰 키 | 파일 단위 blob SHA | 분할 커밋 지원 + 정확도 유지 |
| 저장 구조 | `covered.tsv` + `passes/*.json` 분리 | 자유 텍스트 오매치 차단 |
| 실패 모드 | fail-open | 버그가 레포를 벽돌로 만들면 안 됨 |
| 이름 | `kkochikkochi` / `KkochiKkochi` | 의미 정확, 충돌 0 |
