...
5. **`pending-set.sh` 의 출력은 축소되지 않는다.** 출력은 언제나 `git diff --cached --raw` 를 포함하고, 인식하지 못하는 옵션을 보면 워크트리까지 합집합으로 낸다. 다만 **자동인 것은 스테이징 하한뿐이고, 그 위는 손으로 관리하는 옵션 화이트리스트가 결정한다.** 목록에 옵션을 추가하는 것은 스타일이 아니라 안전성 판단이다 — 먼저 *"이 옵션이 무엇이 커밋되는지 바꾸는가?"* 에 답하라. 바꾼다면 목록에 넣지 말고 `unknown` 으로 떨어뜨린다. (`--pathspec-from-file` 이 잘못 등록되어 실제로 구멍이 났다.)
6. **스킬은 훅이 넘긴 커맨드를 그대로 쓴다.** `"git commit"` 으로 대체하면 영구 교착이 난다. 셸에 넣을 때 `'` → `'\''` 이스케이프는 반드시 한다 — 내용을 바꾸는 것이 아니라 인용 문제이고, 빠뜨리면 문법 오류로 커밋이 영구히 막힌다
