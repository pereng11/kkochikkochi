...
#
# jq 가 없으면 cmd 가 비어 아래 프리필터에서 조용히 빠져나간다. 그래도
# 괜찮다 — deny() 자체가 jq 로 판정 JSON 을 만들므로 jq 없이는 애초에
# 거부할 방법이 없다. 그 환경은 설치기가 설치를 거부해서 막는다 (D42).
cmd="$(jq -r '.tool_input.command // ""' <<<"$payload" 2>/dev/null || echo "")"
# 프리필터. 단순 문자열 포함 검사이므로 `git log --grep=commit` 처럼 커밋을
# 만들지 않는 명령도 여기를 통과해, 미설치 저장소에서는 설치 안내로 한 번
# 거부당한다. D29 가 명시적으로 감수하는 오탐이다 — 오탐은 안내 한 번,
# 미탐은 게이트가 조용히 없는 것이다.
case "$cmd" in
  *commit*) ;;
  *) exit 0 ;;
