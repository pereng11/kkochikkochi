...
플래그 없이 다시 시도하세요. 정말 건너뛰어야 한다면 사용자에게 확인한 뒤, 에이전트가 다시 시도하지 말고 사용자가 직접 그 명령을 실행하게 하세요 — 에이전트의 재시도는 똑같이 거부됩니다."
}
...
#
# ── F2: git 토큰 없이는 --no-verify/-n 판정을 하지 않는다 ──────────────
# *commit*|*push* 프리필터를 문자열 포함 검사로 유지한 대가(D29)가 이 둘로
# 넓힌 뒤 두 배가 됐다: JS/TS 저장소에서 `grep -rn push src/`, `rg -n push`,
# `find . -name '*push*'` 처럼 커밋·push 와 무관한데 "push"·"commit" 이란
# 글자와 짧은 -n 묶음을 우연히 담는 명령이 실측으로도 흔하다. 그래서
# --no-verify/-n 판정에는 명령에 git 호출로 보이는 토큰이 있을 때만
# 들어가게 한 겹을 더 둔다. 아래 마커 쓰기·건강검진(다음 case 블록)에는
# 이 토큰 요건을 적용하지 않는다 — 섞으면 바로 위 문단이 설명한 D44 가
# 재발한다.
#
# 별칭을 길게 늘어놓지 않는다. 실측: `gc --no-verify`, `gcmsg 'x'
# --no-verify`, `gp --no-verify` 는 이미 오늘도 통과한다 — *commit*|*push*
# 프리필터 자체가 "commit"·"push" 글자를 담지 않는 이 서브커맨드 별칭들을
# 애초에 들여보내지 않기 때문이다. 그러니 그런 별칭을 토큰 집합에 넣어도
# 아무것도 달라지지 않는다. git 토큰 요건이 새로 막는 것은 "바이너리는
# 별칭인데 서브커맨드는 풀어 쓴" 형태(`g commit --no-verify`, `g push -n`)
# 뿐이므로, 토큰 집합은 `git` 과 `g` 로 완전하다 — 더 늘릴 이유가 없다.
#
# 토크나이저는 들이지 않는다 (D30) — short_n_re 와 같은 유계 정규식
# 스타일을 그대로 쓴다. 앞 경계는 "[A-Za-z0-9_.-] 가 아닌 문자, 또는
# 문자열 시작"이라 공백·`/`·따옴표·`;`·`&`·`|` 뒤 어디서든 걸린다
# (`/usr/bin/git commit`, `git -C /path commit`,
# `bash -c "git commit --no-verify"` 실측 확인). 뒤 경계도 같은 집합이라
# `digit`·`legit`·`gitlab` 처럼 낱말 안에 파묻힌 경우는 걸리지 않는다.
# 경계 집합에서 `.` 을 일부러 뺐다 — 그래서 `.github/` 안의 "git" 은 앞
# 경계(`.` 바로 뒤)와 뒤 경계(`h` 앞) 양쪽에서 걸러진다(이중 확인,
# `grep -rn push .github/` 가 걸리지 않아야 함을 실측).
#
# 이 겹을 더한 대가를 정직하게 적는다: `git`·`g` 가 아닌 다른 git 별칭
# (예: `gc`, `gcmsg`, 사용자가 셸에 만든 임의의 별칭)으로 --no-verify 를
# 곁들여 commit/push 하는 경우는 이제 이 검사가 못 잡는다. 좁지만 실재하는
# 손실이고, 사람이 받아들였다 — README 한계 표에도 적어 둔다.
git_token_re='(^|[^A-Za-z0-9_.-])(git|g)([^A-Za-z0-9_.-]|$)'
case "$cmd" in
  *commit*|*push*)
    if [[ "$cmd" =~ $git_token_re ]]; then
      case "$cmd" in
