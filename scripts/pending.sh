...
if [ "$MODE" != "current" ]; then
  ...
  # 보인다. 종료 코드는 두 경우 다 1로 그대로 둔다: 호출자는 둘 다
  # "지금 검증할 대상 없음"으로만 취급하면 되고, 코드가 바뀌면 그 계약이
  # 깨진다. (빈 원장은 여기를 통과해 아래 ledger_unverified 로 가고,
  # out="" 이 되어 그다음 줄의 "검증할 것이 없습니다"로 자연히 떨어진다 —
  # 손상이 아니다.)
  if [ ! -r "$ledger" ]; then
