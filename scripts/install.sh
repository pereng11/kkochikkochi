...
cmd_status() {
  ...
  for name in $HOOK_NAMES; do
    ...
    # 재설치를 영원히 요구하게 된다.
    #
    # 하지만 조용히 넘기지는 않는다. 플러그인 캐시가 (uninstall→install
    # 왕복이나 손상된 배포로) 이 훅의 원본만 읽지 못하게 되면, 이 판정을
    # 건너뛴 채로 나머지 훅만 보고 0을 낼 수 있다 — 그 훅 층이 실제로는
    # 없거나 낡았는데도 상태는 "정상"으로 보고되는 것이다. exit 코드는
    # 그대로 두되(위 이유), 이 사실만은 stderr 에 남긴다.
    if [ ! -r "$src" ]; then
      echo "kkochikkochi: 경고 — $name 훅 원본을 읽을 수 없어 최신 여부를 판정하지 못했습니다 ($src). 이 훅 층이 실제로는 빠져 있거나 낡았어도 상태는 정상으로 보일 수 있습니다." >&2
      continue
    fi
