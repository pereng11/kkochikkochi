...
tokenize_cmd() {
  ...
  # 묶여 사실상 선형이 된다.
  #
  # 상태 기계라 따옴표·단어는 조각 경계를 그냥 넘어간다. 유일한 예외가
  # 2글자 연산자 `&&` `||` 다. 경계에 걸친 `&`/`|` 를 **하나만** 다음
  # 조각으로 넘기면 오히려 버그가 난다: 조각이 `...&&` 로 끝날 때 뒤의
  # `&` 만 넘기면 앞의 `&` 는 이 조각에서 c2 룩어헤드가 1글자밖에 못 봐
  # 평범한 글자로 처리되고, 넘긴 `&` 도 다음 조각에서 똑같이 처리되어
  # **세그먼트 구분자가 통째로 사라진다.** 그래서 경계에 붙은 `&`/`|` 는
  # 연속된 것을 전부 넘긴다. 그러면 연산자는 언제나 한 조각 안에서 온전히
  # 보인다.
  while [ -n "$rest" ]; do
    buf="${rest:0:1024}"
    rest="${rest:1024}"
    if [ -n "$rest" ]; then
      blen=${#buf}
      # blen > 1 조건은 buf 가 비어 무한 루프가 되는 것을 막는다
      # (1024글자가 전부 &/| 인 경우에만 걸리는 이론적 경계).
      while [ "$blen" -gt 1 ]; do
        last="${buf:$((blen - 1)):1}"
        case "$last" in
          "&" | "|")
